...
class YourDigitsSupabaseUser extends BaseAuthUser {
  YourDigitsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> yourDigitsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = YourDigitsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
